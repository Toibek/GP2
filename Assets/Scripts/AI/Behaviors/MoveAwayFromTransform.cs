...
public class MoveAwayFromTransform : Node
{
    ...
    public MoveAwayFromTransform(Rigidbody rigidbody, float runSpeed,float runLength, string moveToPositionSaveVariable = "RequestedNewPosition", float smoothDampSpeed = 0.6f) : base() 
    {
        _rb = rigidbody;
        _runSpeed = runSpeed;
    ...
    public override NodeState Evaluate()
    {
        ...
        if (RunFromTransform != null)
        {
            ...
            GetRootNode().SetData(_moveToPositionSaveVariable, _rb.position + (newPosFromRigidbody * _runLength));
